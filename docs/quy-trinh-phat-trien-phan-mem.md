...
**Output:** `projects/<slug>/docs/requirements/<slug>.md` (điền theo `templates/user-story.md`) — gồm mục tiêu, phạm vi (trong/ngoài), người dùng mục tiêu, tiêu chí chấp nhận (acceptance criteria) dạng Given/When/Then.

**Cách dùng Claude Code:** `/requirements <mô tả ngắn>`. Claude sẽ hỏi lại các điểm chưa rõ (phạm vi, ràng buộc, ưu tiên) thay vì tự đoán, rồi ghi file. Dừng lại, không tự chuyển sang thiết kế.

## Giai đoạn 2 — Thiết kế kiến trúc

**Input:** File requirements đã duyệt ở giai đoạn 1.

**Output:** `projects/<slug>/docs/adr/NNN-<slug>.md` (điền theo `templates/adr-template.md`) — ghi lại 2-3 phương án đã cân nhắc, lý do chọn, trade-off, hệ quả.

**Cách dùng Claude Code:** `/design <slug>`. Nên bật **Plan Mode** khi thảo luận để Claude không sửa code trong lúc chỉ đang bàn kiến trúc. Dừng lại chờ duyệt ADR trước khi lập kế hoạch task.
...
**Output:** `projects/<slug>/docs/retro/<ngày>.md` (bài học rút ra); bugfix mới đi qua lại `/new-feature` (hoặc skill cá nhân có sẵn) rồi quay lại giai đoạn 6-8.

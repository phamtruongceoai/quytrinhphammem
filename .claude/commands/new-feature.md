...
Kết thúc: tóm tắt file đã đổi, nhắc bước tiếp theo là `/code-review` (và `/security-review` nếu task đụng auth/dữ liệu người dùng). Nếu task vừa làm được đánh dấu **rủi ro Cao** trong task breakdown, đề nghị chạy `/code-review` ngay bây giờ trước khi sang task tiếp theo, thay vì gộp review sau nhiều task.

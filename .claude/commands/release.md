...
Đây là giai đoạn 8 (docs/quy-trinh-phat-trien-phan-mem.md). $ARGUMENTS gồm `<project-slug>` + số version mới (semver, vd `todo-cli 0.2.0`). Nếu chỉ nhận được số version mà không rõ project nào, hỏi lại người dùng `<project-slug>` trước khi tiếp tục. Version áp dụng cho toàn bộ project con, không phải riêng 1 tính năng.
...
5. Viết release notes vào `projects/<project-slug>/docs/release/<version>.md`, dựa theo `templates/release-notes-template.md`.
...
- Tag phải gắn `<project-slug>` — dạng `<project-slug>-v<version>` (vd `todo-cli-v0.2.0`) — vì repo chứa nhiều project con dùng chung một lịch sử git, tag trần `v<version>` sẽ đụng nhau giữa các project.
- KHÔNG tự tạo git tag hoặc push. Chỉ chuẩn bị thay đổi, hiện toàn bộ ra để người dùng xác nhận.

Kết thúc: hiện CHANGELOG/version/release notes đã soạn, hỏi rõ ràng người dùng có xác nhận để tạo tag `<project-slug>-v<version>` không.

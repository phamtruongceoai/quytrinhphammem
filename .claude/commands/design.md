...
Đây là giai đoạn 2 (docs/quy-trinh-phat-trien-phan-mem.md). Input là file requirements đã có ở `projects/<project-slug>/docs/requirements/<feature-slug>.md`. Nếu chưa có, dừng lại và đề nghị chạy `/requirements` trước.
...
4. Ghi ADR vào `projects/<project-slug>/docs/adr/NNN-<feature-slug>.md` (NNN tăng dần — liệt kê thư mục `docs/adr/` của project này để xem file lớn nhất đã có, xác định số tiếp theo), dựa theo `templates/adr-template.md`.
